...
globalThis.HeroCarousel = class HeroCarousel {
  currentSlide = 0;
  slides = [];
  dots = [];
  ...
  createNavigation() {
    ...
    this.slides.forEach((_, index) => {
      ...
      dot.setAttribute(
        "aria-label",
        `Ir al slide ${index + 1} de ${this.slides.length}`,
      );
...
globalThis.initHeroCarousel = function () {
  if (globalThis.HeroCarousel == "undefined") {
    return;
  }

  const carousel = new globalThis.HeroCarousel();
  carousel.init();

  // Exponer para debugging
  if (globalThis.location.hostname === "localhost") {
    globalThis.debugCarousel = carousel;
  }
};
